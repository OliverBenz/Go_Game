...
cv::Mat DebugVisualizer::buildMosaic() {
    ...
    for (int c = 0; c < cols; ++c) {
        ...
        const std::string stageName = stage.name.empty()
            ? "Stage " + std::to_string(c + 1)
            : stage.name;
        const std::string headerText = stageName + " (" + std::to_string(stage.images.size()) + ")";
        cv::putText(header, headerText, cv::Point(8, STAGE_HEADER_H - 10),
            cv::FONT_HERSHEY_SIMPLEX, 0.75, HEADER_FG, 1, cv::LINE_AA);
    }
    ...
    for (size_t r = 0; r < maxSteps; ++r) {
        ...
        for (int c = 0; c < cols; ++c) {
            ...
            cv::putText(cell, step.name, cv::Point(TILE_PAD, 20),
                cv::FONT_HERSHEY_SIMPLEX, 0.55, HEADER_FG, 1, cv::LINE_AA);
            ...
            const double scale = std::min(
                static_cast<double>(availW) / static_cast<double>(vis.cols),
                static_cast<double>(availH) / static_cast<double>(vis.rows)
            );
...
cv::Mat DebugVisualizer::labelTile(const cv::Mat& tile, const std::string& text) {
    ...
    cv::putText(out, text, cv::Point(pad, 20),
